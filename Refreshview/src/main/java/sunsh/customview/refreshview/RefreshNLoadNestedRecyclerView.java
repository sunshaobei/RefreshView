...
import sunsh.customview.refreshview.hfrvnested.PullToLoad.PullToLoadRecyclerView;
 ...
 * </p>
 */
public class RefreshNLoadNestedRecyclerView extends LinearLayout implements NestedScrollingParent,
    ...
    public RefreshNLoadNestedRecyclerView(Context context, AttributeSet attrs) {
        super(context, attrs);
        PullToLoadRecyclerView pullToLoadRecyclerView = new PullToLoadRecyclerView(context);
    ...
    public PullToLoadRecyclerView getRv() {
        return getLoadMore();
    }
    ...
    public void setBottomRefreshing(boolean refreshing, int loadCount) {
        if (refreshing) {
            getLoadMore().HsetState(getLoadMore().mLoadViewHeight);
        } else {
...
//    }
    ...
    public boolean onStartNestedScroll(View child, View target, int nestedScrollAxes) {
        ...
        return isEnabled() && getRefresh().HmState == PullToRefreshRecyclerView.STATE_DEFAULT && getLoadMore().FmState == PullToLoadRecyclerView.STATE_DEFAULT //没有在刷新和返回途中
    ...
    public void onNestedPreScroll(View target, int dx, int dy, int[] consumed) {
        ...
        if (dy < 0 && mTotalUnconsumedBottom > 0 && getLoadMore().FmState != PullToLoadRecyclerView.STATE_LOADING) {
    ...
    public void onStopNestedScroll(View target) {
        ...
        if (mTotalUnconsumed > 0 && getLoadMore().HmState != PullToLoadRecyclerView.STATE_LOADING) {
    ...
    private PullToLoadRecyclerView getLoadMore() {
        return ((PullToLoadRecyclerView) getChildAt(0));
    }
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (mIsBeingDragged) {
                    ...
                    if (overscrollTop > 0 && getLoadMore().FmState != PullToLoadRecyclerView.STATE_LOADING) {
            ...
            case MotionEvent.ACTION_UP: {
                ...
                if (mIsBeingDragged && getLoadMore().FmState != PullToLoadRecyclerView.STATE_LOADING) {
                    final float y = ev.getY(pointerIndex);
                    final float overscrollTop = (y - mInitialMotionY) * DRAG_RATE;
                    mIsBeingDragged = false;
    ...
    public interface OnChildScrollUpCallback {
         ...
         */
        boolean canChildScrollUp(RefreshNLoadNestedRecyclerView parent, @Nullable View child);
    }
}
